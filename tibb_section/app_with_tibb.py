...
class Storage:
    ...
    def update_score(self, uid, typ, wpm, device_type=None, accuracy=100.0):
        ...
        # التحقق من أن المستخدم اختار تخصصه (device_type)
        # إذا لم يختر تخصصه، لا تحفظ السرعة في الصدارة
        if device_type is None:
            print(f"[SCORE] User {uid} has no device_type - speed not saved")
            return

        # التحقق من أن الدقة 100% أو أكثر
        # لا تحفظ أي سرعة بدقة أقل من 100%
        if accuracy < 100.0:
            print(f"[SCORE] User {uid} accuracy {accuracy}% < 100% - speed not saved")
            return

        # التحقق من الحد الأقصى للسرعة
        # مس و جب: 210
        # باقي الأقسام: 190
        max_speed = 210 if typ in ["مس", "جب"] else 190
        
        if wpm > max_speed:
            # السرعة تخطت الحد - لا تحفظها
            print(f"[SCORE] User {uid} speed {wpm:.1f} exceeds limit {max_speed} - speed not saved")
            return
    ...
    def is_speed_over_limit(self, typ, wpm):
        """التحقق من أن السرعة تخطت الحد الأقصى"""
        max_speed = 210 if typ in ["مس", "جب"] else 190
        return wpm > max_speed

    def get_speed_limit(self, typ):
        """الحصول على الحد الأقصى للسرعة لقسم معين"""
        return 210 if typ in ["مس", "جب"] else 190

    def save_pending_speed(self, uid, typ, wpm, device_type, username=None):
        """حفظ سرعة معلقة بانتظار موافقة المالك"""
        uid_str = str(uid)
        if "pending_speeds" not in self.data:
            self.data["pending_speeds"] = {}
        
        key = f"{uid}_{typ}_{device_type}"
        self.data["pending_speeds"][key] = {
            "uid": uid,
            "typ": typ,
            "wpm": wpm,
        ...
        self.mark_dirty()
        self.save(force=True)
        print(f"[PENDING] Saved pending speed for user {uid} - {typ} - {wpm:.1f} WPM")

    def get_pending_speed(self, uid, typ, device_type):
        """الحصول على سرعة معلقة"""
        uid_str = str(uid)
        if "pending_speeds" not in self.data:
            return None
        key = f"{uid}_{typ}_{device_type}"
        return self.data["pending_speeds"].get(key)

    def approve_pending_speed(self, uid, typ, device_type):
        """قبول سرعة معلقة وحفظها في الصدارة"""
        uid_str = str(uid)
        if "pending_speeds" not in self.data:
            return False
        
        key = f"{uid}_{typ}_{device_type}"
        pending = self.data["pending_speeds"].get(key)
        if not pending:
            return False
        
        # حفظ السرعة في الصدارة (بypass الحد الأقصى)
        score_key = f"{uid}_{typ}_{device_type}"
        old_score = self.leaderboard_data["scores"].get(score_key, 0)
        wpm = pending["wpm"]
        self.leaderboard_data["scores"][score_key] = max(old_score, wpm)
        
        # حذف من المعلقة
        del self.data["pending_speeds"][key]
        
        self.mark_dirty()
        self.save(force=True)
        self.save_leaderboard()
        print(f"[APPROVED] User {uid} - {typ} - {wpm:.1f} WPM approved and saved")
        return True

    def reject_pending_speed(self, uid, typ, device_type):
        """رفض سرعة معلقة وحذفها"""
        uid_str = str(uid)
        if "pending_speeds" not in self.data:
            return False
        
        key = f"{uid}_{typ}_{device_type}"
        if key in self.data["pending_speeds"]:
            del self.data["pending_speeds"][key]
            self.mark_dirty()
            self.save(force=True)
            print(f"[REJECTED] User {uid} - {typ} - speed rejected")
            return True
        return False

    def get_all_pending_speeds(self):
        """الحصول على جميع السرعات المعلقة"""
        if "pending_speeds" not in self.data:
            return {}
        return self.data["pending_speeds"]

    def find_pending_by_username(self, username):
        """البحث عن سرعة معلقة باليوزر"""
        if "pending_speeds" not in self.data:
            return []
        
        username = username.lstrip("@")
        results = []
        for key, data in self.data["pending_speeds"].items():
            if data.get("username", "").lstrip("@") == username:
...
async def handle_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # معالج أمر "سماح @username" للمالك الأساسي
    if text.startswith("سماح "):
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر للمالك الأساسي فقط")
            return
        
        # استخراج اليوزر
        username = text.replace("سماح", "").strip().lstrip("@")
        if not username:
            await u.message.reply_text("الاستخدام: سماح @username")
            return
        
        # البحث عن السرعات المعلقة لهذا اليوزر
        pending_list = storage.find_pending_by_username(username)
        if not pending_list:
            await u.message.reply_text(f"لا توجد سرعات معلقة لـ @{username}")
            return
        
        # قبول جميع السرعات المعلقة
        approved_count = 0
        for key, data in pending_list:
            success = storage.approve_pending_speed(data["uid"], data["typ"], data["device_type"])
            if success:
                approved_count += 1
        
        if approved_count > 0:
            await u.message.reply_text(f"تم قبول {approved_count} سرعة معلقة لـ @{username} وحفظها في الصدارة")
        else:
            await u.message.reply_text(f"فشل في قبول السرعات المعلقة لـ @{username}")
        storage.log_cmd("سماح")
        return

    # معالج أمر "لا" للمالك الأساسي - رفض جميع السرعات المعلقة لآخر يوزر
    if text == "لا":
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر للمالك الأساسي فقط")
            return
        
        # حذف جميع السرعات المعلقة
        pending_count = len(storage.get_all_pending_speeds())
        if pending_count == 0:
            await u.message.reply_text("لا توجد سرعات معلقة حالياً")
            return
        
        # حذف جميع السرعات المعلقة
        storage.data["pending_speeds"] = {}
        storage.mark_dirty()
        storage.save(force=True)
        await u.message.reply_text(f"تم رفض وحذف {pending_count} سرعة معلقة")
        storage.log_cmd("لا")
        return
    ...
    if best_match:
        ...
        if is_fully_accepted:
            ...
            if score_typ in leaderboard_sections:
                ...
                if score_typ == "جب" and user_jab_type.get(uid) == "حرفين":
                    ...
                    # التحقق من تخطي الحد الأقصى للسرعة
                    if storage.is_speed_over_limit(score_typ, wpm):
                        # السرعة تخطت الحد - احفظها كمعلقة وأرسل إشعار للمالك
                        max_speed = storage.get_speed_limit(score_typ)
                        storage.save_pending_speed(uid, score_typ, wpm, device_type, username=usr)
                        
                        # إرسال إشعار للمالك
                        try:
                            owner_msg = f"فيه واحد جاب رقم @{usr if usr else name} {wpm:.2f} بكيفك تبي تسمح له ولا لا؟"
                            await c.bot.send_message(chat_id=5562144078, text=owner_msg)
                        except Exception as e:
                            print(f"[PENDING] Failed to notify owner: {e}")
                    else:
                        # السرعة ضمن الحد - احفظها مباشرة
                        storage.update_score(uid, score_typ, wpm, device_type, accuracy=accuracy if accuracy is not None else 100.0)
        ...
        if round_data:
            ...
            if is_fully_accepted:
                ...
                # التحقق من تخطي الحد الأقصى
                leaderboard_sections = ["كرر", "ويكي", "مس", "شك", "جب", "جش", "جمم"]
                if score_typ in leaderboard_sections and storage.is_speed_over_limit(score_typ, wpm):
                    # السرعة تخطت الحد - أرسل رسالة الحجب
                    reply = f"سرعتك هي {wpm:.2f}\n\nالرقم ماهو رقم جوال فا عشان كذا انحجب من التوب قدم دليل لهذه الحساب (iv511vi) عشان ينحسب رقمك"
                else:
                    # السرعة ضمن الحد - أرسل رسالة عادية
                    reply = f"كفو يا {mention}\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed:.2f} ثانية"
                    if accuracy is not None and user_accuracy_enabled:
                        reply += f"\nالدقة : {accuracy}%"
                await u.message.reply_text(reply)
